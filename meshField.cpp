...
{
	// 各種値のクリア
	strcpy(m_aBinaryFileName, "\0");            // 頂点情報が格納されたバイナリファイル名
...
//    生成処理
//=============================================================================
CMeshField *CMeshField::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, D3DXCOLOR col, float fWidth, float fHeight, int nXBlock, int nZBlock, int nTexSplitU, int nTexSplitV, char *pFileName, int nPriority)
{
	...
	{// まだ生成できる
		...
		{// メモリが空になっている
			...
			{// インスタンスを生成できた
				...
				pMeshField->SetTexSplitU(nTexSplitU);      // テクスチャU座標の分割数
				pMeshField->SetTexSplitV(nTexSplitV);      // テクスチャV座標の分割数
	...
	{// レンダリングクラスが生成されている
		...
		{// デバイスが取得できた
			...
			{// 垂直方向の分割数 + 1だけ繰り返し
				...
				fTexV += ((1.0f / m_nZBlock) * m_nTexSplitV);                     // テクスチャV座標を進める
