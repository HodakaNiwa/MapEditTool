...
void CEmitter::Uninit(void)
{
	...
	if (pEditer != NULL)
	{
		if (m_bLoop == false)
		{
			pEditer->SetEffectState(CEditer::EFFECTSTATE_STOP);
		}
	}
...
void CParEmitter::SetEffect(void)
{
	...
	{// メモリが確保されている
		...
		// パーティクルの生成
		CParticle *pParticle = CParticle::Create(pos, Move, ChangeMove, InitCol, fWidth, fHeight, fRot, ChangeCol, fGravity, fSpread, fChangeRot, fBouncing, nLife, nRotPattern, bCollision, bDrawAddtive, bLighting, EMITTER_EFFECT_PRIORITY);
		if (pParticle != NULL)
		{// パーティクルの生成に成功した
