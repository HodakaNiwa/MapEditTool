...
void CEffectManager::SetEffect(D3DXVECTOR3 pos, D3DXVECTOR3 rot, int nType, int nPriority)
{
	...
	{// データクラスが作成されている
		...
		{// 0番のエフェクトの種類(パーティクル)だったら
			m_pEmitter = CParEmitter::Create(pos, rot, nType, nTexIdx, nEffectIdx, fRangeMax, fRangeMin, nLife, nLap, nAppear, bLoop, m_apParData[nEffectIdx], m_pTextureManager->GetTexture(nTexIdx), nPriority);
		}
		else if (nEffectType == 1)
		{// 1番のエフェクトの種類(リングエフェクト)だったら
			m_pEmitter = CRingEmitter::Create(pos, rot, nType, nTexIdx, nEffectIdx, fRangeMax, fRangeMin, nLife, nLap, nAppear, bLoop, m_apRingData[nEffectIdx], m_pTextureManager->GetTexture(nTexIdx), nPriority);
		}
	}
}
...
//    マップ配置用のエフェクトを設定する処理
//=============================================================================
CEmitter *CEffectManager::SetMapEditEffect(D3DXVECTOR3 pos, D3DXVECTOR3 rot, int nType, int nPriority)
{
	CEmitter *pSetEmitter = NULL;
	...
	{// データクラスが作成されている
		...
		if (nEffectType == 0)
		{// 0番のエフェクトの種類(パーティクル)だったら
			pSetEmitter = CSetParEmitter::Create(pos, rot, nType, nTexIdx, nEffectIdx, fRangeMax, fRangeMin, nLife, nLap, nAppear, bLoop, m_apParData[nEffectIdx], m_pTextureManager->GetTexture(nTexIdx), nPriority);
		}
		else if (nEffectType == 1)
		{// 1番のエフェクトの種類(リングエフェクト)だったら
			pSetEmitter = CSetRingEmitter::Create(pos, rot, nType, nTexIdx, nEffectIdx, fRangeMax, fRangeMin, nLife, nLap, nAppear, bLoop, m_apRingData[nEffectIdx], m_pTextureManager->GetTexture(nTexIdx), nPriority);
		}
	}
